...
def get_buildings(path_to_buildings):
    ...
    for level in final_clusters.columns:
        buildings[f"level_{level}"] = buildings.level_7_label.map(final_clusters[level])
        level_columns.append(f"level_{level}")
    # Rename lowest level to level 7
    level_7 = buildings.pop("level_7_label")
...
def compute_bc_matrix(count_table):
    ...
    bray_curtis_df = pd.DataFrame(
        bray_curtis_matrix, index=count_table.T.columns, columns=count_table.T.columns
    )
